...
class DaftSex : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        val items: List<SearchResponse> = when (request.data) {
            ...
            // Latest Videos
            "latest" -> {
                val url = if (page <= 1) "$mainUrl/fresh" else "$mainUrl/fresh/$page"
                val doc = app.get(url, headers = defaultHeaders).document
                doc.select("a[href*='/movie/'], div.movie-item a").mapNotNull { parseMovieCard(it) }
            }
            ...
            // Actors / Models
            "actors" -> {
                val url = if (page <= 1) "$mainUrl/models" else "$mainUrl/models/$page"
                val doc = app.get(url, headers = defaultHeaders).document
            ...
            // Studios / Channels
                ...
                val doc = app.get(url, headers = defaultHeaders).document
                doc.select("a[href*='/channel/'], a[href*='/movie/']").mapNotNull { parseMovieCard(it) }
            }
        }
    ...
    override suspend fun search(query: String): List<SearchResponse> = coroutineScope {
        ...
        // 1. Search actors (Synthetic Performer Card + Model Search)
        val actorsJob = async {
            runCatching {
